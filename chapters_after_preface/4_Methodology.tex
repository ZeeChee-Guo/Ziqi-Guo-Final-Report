...
This chapter describes the methodological workflow used in this project to detect anomalies in time series with concept drift. The chapter builds on the prior AnDri method, which provides the dynamic normal-pattern model and the anomaly scoring mechanism \citep{park2025andri,park2025adaptive}. The focus here is the end-to-end detection methodology implemented in the demo system: score generation, normal-pattern-aware thresholding, active-learning-based label refinement, and final anomaly decision making. While anomaly scores and normal-pattern assignments are obtained from existing detectors such as AnDri, the adaptive threshold refinement framework, Bayesian mixture-based threshold estimation, active-learning candidate selection strategy, and pattern-specific threshold assignment were designed and implemented in this project.
...
\[g(i) \in \{1,2,\ldots,m\},\]
where \(g(i)\) indicates the normal pattern that best matches point \(i\). AnDri sets thresholds separately for different normal patterns instead of using a single global threshold.
...
When two active Gaussian components are found, it means the current user labels do not yet separate normal and abnormal score regions clearly. The system then computes the intersection between adjacent Gaussian components:
...
The AnDri estimates a separate threshold for each normal pattern from the training set. Let \(A\) be the set of user-selected training indices. For each normal pattern \(k\), the training indices assigned to that pattern are
...
and the missing threshold is assigned as \(\theta_k=\theta_{k^*}\).
